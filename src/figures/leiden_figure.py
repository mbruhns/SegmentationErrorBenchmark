...
def main():
    ...
    df_lst = []
    partial_df_path = Path("results/Leiden/")

    for df_path in partial_df_path.rglob("*.feather"):
        df_lst.append(pd.read_feather(df_path))

    leiden_df = pd.concat(df_lst)
    leiden_df["variance"] = leiden_df.variance.replace(
    ...
    sns.barplot(
        data=leiden_df.loc[leiden_df.variance == "None"],
        x="k",
        y="arand",
        hue="perturbation",
        ax=axd["A"],
    ...
    sns.barplot(
        data=leiden_df.loc[leiden_df.variance == "log1p"].copy(),
        x="k",
        y="arand",
        hue="perturbation",
        ax=axd["C"],
    ...
    plt.savefig("figures/leiden_arand.pdf", transparent=True)
    plt.close()
