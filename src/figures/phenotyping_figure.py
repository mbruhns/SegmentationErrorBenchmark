...
def main():
    ...
    for pert_strength in [70, 80, 90, 100]:
        ...
        for grouping_id, mapping in enumerate(
            ...
            # Get maximum of off-diagonal elements using Numpy functions
            max_off_diag = np.max(cm_df.values[~np.eye(len(cm_df), dtype=bool)])
            print(max_off_diag)
            ...
            # Overlay the diagonal with grey squares and white annotations
            for i in range(len(cm_df)):
                # Add grey rectangle for the diagonal cell
                ax.add_patch(
                    patches.Rectangle(
                        (i, i), 1, 1, fill=True, color="grey", ec="grey", lw=0
                    )
                )
                # Annotate the diagonal in white if value is above threshold
                val = cm_df.iloc[i, i]
                if val >= 0.01:
                    ax.text(
                        i + 0.5,
