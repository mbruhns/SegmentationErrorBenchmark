...
@njit
def separation_border_old(mask):
    adjacency_matrix, nn_arr = cell_adjacency_numba(label_mask=mask)

    cell_ids = np.argwhere(adjacency_matrix) + 1
    idx = np.random.choice(np.array([0, 1]), size=cell_ids.shape[0]).astype("uint8")
    remove_ids = np.where(idx, cell_ids[:, 0], cell_ids[:, 1]).astype("uint16")
    k = np.where(nb_isin(a=nn_arr * mask, b=remove_ids), 0, 1).astype("int32")

    return k * mask


key_type = numba.types.uint32
val_type = numba.types.ListType(numba.types.uint32)


@njit
def separation_border(label_mask):
    cell_num = np.max(label_mask) + 1

    d = numba.typed.Dict.empty(key_type, val_type)

    for i in range(1, cell_num):
        d[i] = numba.typed.List.empty_list(numba.types.uint32)

    row_num, col_num = label_mask.shape
    np.zeros_like(label_mask, dtype="uint8")

    label_mask_border = pad_numba(label_mask, pad_size=1)
    nn_arr = np.zeros_like(label_mask, dtype="uint8")
    nzr = np.nonzero(label_mask_border)

    for i, j in zip(nzr[0], nzr[1]):
        cur_val = label_mask_border[i, j]
        temp_matrix = np.array(
            [
                label_mask_border[i - 1, j],
                label_mask_border[i + 1, j],
                label_mask_border[i, j - 1],
                label_mask_border[i, j + 1],
            ]
        )
        ...
        if unique_nn.size > 1:
            ids = numba_setdiff(
                arr1=unique_nn, arr2=np.array([0, cur_val], dtype="uint32")
            )

            if ids.size > 0:
                nn_arr[i - 1, j - 1] = 1
                for neighbor_id in ids:
                    d[cur_val].append(neighbor_id)

    # Process cell_ids and remove_ids based on adjacency list
    cell_ids = []
    for cell, neighbors in d.items():
        for neighbor in neighbors:
            cell_ids.append([cell, neighbor])
    cell_ids = np.array(cell_ids)

    # Randomly select cells from the adjacency list for removal
    idx = np.random.choice(np.array([0, 1]), size=cell_ids.shape[0]).astype("uint8")
    remove_ids = np.where(idx, cell_ids[:, 0], cell_ids[:, 1]).astype("uint16")

    k = np.where(nb_isin(a=nn_arr * label_mask, b=remove_ids), 0, 1).astype("int32")

    # Final perturbed mask
    perturbed_mask = k * label_mask
    return perturbed_mask


@njit(parallel=True, fastmath=True)
def separation_border_inplace_old(image):
    x, y = image.shape
    ...
    for i in prange(1, x - 1):
        for j in range(1, y - 1):
            ...
            if (
                (image[i - 1, j] != 0 and image[i - 1, j] != cur_val)
                or (image[i + 1, j] != 0 and image[i + 1, j] != cur_val)
                or (image[i, j - 1] != 0 and image[i, j - 1] != cur_val)
                or (image[i, j + 1] != 0 and image[i, j + 1] != cur_val)
            ):
                image[i, j] = 0
...
def main():
    ...
    csv_file = f"results/output_{current_id}.csv"
